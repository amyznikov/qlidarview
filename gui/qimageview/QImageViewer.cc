...
void QImageViewer::setDisplayFunction(c_image_display_function *  displayfunc)
{
  this->display_function_ = displayfunc;
}

c_image_display_function * QImageViewer::displayFunction() const
{
...
void QImageViewer::createDisplayImage()
{
  if ( currentImage_.empty() ) {
    ...
    if ( !display_function_ ) {
      ...
      displayImage_.release();
      display_function_->create_display_image(currentImage_, currentMask_,
          displayImage_, currentImage_.depth());
    }
    else  {
      displayImage_.release();
      display_function_->create_display_image(currentImage_, currentMask_,
...
void QImageViewer::copyDisplayImageToClipboard()
{
  ...
  if ( !clipboard ) {
    QMessageBox::critical(this, "ERROT", "No application clipboard available");
  }
  else {
