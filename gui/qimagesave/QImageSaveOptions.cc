    ...
    const QString & fileName)
{
  ...
  CF_DEBUG("currentImage.channels()=%d depth=%d", currentImage.channels(), currentImage.depth());
  ...
  while ( 42 ) {
    ...
    if ( format == QImageSaveFLO ) {
      ...
      CF_DEBUG("QImageSavePNG selectedPixelDepth=%d", selectedPixelDepth);
      if ( currentImage.channels() == 2 ) {  // optical flow
        flow2HSV(currentImage, image, 0, true);
      }
      else if ( currentImage.depth() == selectedPixelDepth ) {
        CF_DEBUG("QImageSavePNG image = currentImage");
        image = currentImage;
      }
      else {
        double minv, maxv;
        ...
        CF_DEBUG("image.convertTo(image, selectedPixelDepth=%d)",selectedPixelDepth);
        image.convertTo(image, selectedPixelDepth);
        CF_DEBUG("image.depth=%d",image.depth());
      }
    }
