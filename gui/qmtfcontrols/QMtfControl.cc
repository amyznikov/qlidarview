...
//  topToolbar_->addWidget(displayChannel_ctl);
  ...
  inputRangeLabel_ctl = new QLabel("Input Range: ", this);
  topToolbar_->addWidget(inputRangeLabel_ctl);

  inputRange_ctl = new QNumberEditBox(this);
  topToolbar_->addWidget(inputRange_ctl);
...
void QMtfControl::updateControls()
{
  if ( !mtf_ ) {
    ...
    mtf_->get_input_range(&minval, &maxval);
    inputRange_ctl->setText(QString("%1;%2").arg(minval).arg(maxval));
