...
{
  static const QString lastSavedPathKeyName =
      "QBrowsePathComboLastSavedPath";
  ...
  if ( path.isEmpty() ) {
    QSettings settings;
    path = settings.value(lastSavedPathKeyName).toString();
  }
  ...
  if ( dlg.exec() == QDialog::Accepted ) {
    ...
    if( !path.isEmpty() ) {
      QSettings settings;
      settings.setValue(lastSavedPathKeyName, path);
    }
